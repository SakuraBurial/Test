...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr titles:(NSArray*)titleArray
{
	...
	if (self != nil)
	{
        ...
        NSLog(@"_tabBar = %@",_tabBar);
		_tabBar.delegate = self;
        
        UIImage * image = [UIImage imageNamed:@"sepline.png"];
        UIButton * btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, _tabBar.frame.size.width, 0.8)];
...
- (void)viewDidLoad
{
    ...
    [_transitionView setClipsToBounds:YES];//SetContentMode属性能够确保图像始终保持原始状态（不会被拉伸），而SetClipsToBounds属性则用来保证图像不会超过对应区块的边界。
